    ...
    <div style="background:#fff;border:1px solid #dfe2e6;border-radius:16px;padding:20px 22px;display:flex;align-items:center;gap:18px">
      ...
      <div style="flex:1;min-width:0">
        <div style="display:flex;align-items:center;gap:11px">
          ...
          <span style="font-family:'IBM Plex Mono',monospace;font-size:9px;letter-spacing:0.08em;padding:3px 9px;border-radius:7px;background:rgba(232,180,8,0.16);color:#caa006">OWNER</span>
        </div>
      ...
      <button style="background:transparent;border:none;cursor:pointer;font-family:inherit;font-size:13px;font-weight:600;color:#6b7280;display:inline-flex;align-items:center;gap:6px;white-space:nowrap;align-self:flex-start">
        Browse other companies <iconify-icon icon="tabler:arrow-right" width="16"></iconify-icon>
      </button>
    </div>
      ...
      <div style="display:grid;grid-template-columns:1.5fr 1fr;gap:14px;align-items:start">
        <div style="background:#fff;border:1px solid #dfe2e6;border-radius:15px;padding:18px">
          <div style="display:flex;align-items:center;justify-content:space-between;margin-bottom:18px">
            ...
            <span style="font-family:'IBM Plex Mono',monospace;font-size:10px;color:#9aa1ab">387 total</span>
          </div>
...
const companyStats = computed(() => [
  ...
  { icon: "tabler:trophy", value: store.config.companyServerRank.split(" ")[0], label: "Server rank", sub: "top 5% of fleets", color: "#8b5cf6", bg: "rgba(139,92,246,0.14)" },
]);
...
const maxChart = computed(() => Math.max(...store.config.statChart.map(b => b.v)));

function barHeight(v: number) {
  return Math.round((v / maxChart.value) * 100) + "%";
}
</script>
