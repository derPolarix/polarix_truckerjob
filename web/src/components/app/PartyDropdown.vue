  ...
  <div ref="rootEl" style="position:relative">
    ...
    >
      <div style="display:flex;align-items:center;justify-content:space-between;padding:14px 16px;border-bottom:1px solid #eef0f2;flex-shrink:0">
        <span style="font-size:14px;font-weight:700;color:#1b1f24">{{ t('app.convoy_title') }}</span>
      ...
      <div style="overflow-y:auto;flex:1;padding:12px 14px;display:flex;flex-direction:column;gap:10px">
        ...
        <div v-if="store.pendingInvite" style="padding:12px;border-radius:12px;background:rgba(232,180,8,0.08);border:1px solid rgba(232,180,8,0.25)">
          <div style="font-size:13px;color:#1b1f24"><span style="font-weight:700">{{ store.pendingInvite.fromName }}</span> {{ t('app.invited_you_to_convoy') }}</div>
          <div style="display:flex;gap:8px;margin-top:10px">
            <button class="accent-btn" style="flex:1;padding:8px;font-size:12px;justify-content:center" @click="respond(true)">{{ t('app.accept') }}</button>
            <button style="flex:1;background:#fff;color:#6b7280;border:1px solid #e4e6e9;border-radius:9px;padding:8px;font-family:inherit;font-weight:600;font-size:12px;cursor:pointer" @click="respond(false)">{{ t('app.decline') }}</button>
          </div>
        </div>
        ...
        <template v-if="store.party">
          <div v-if="store.rewardMultiplier" style="display:flex;align-items:center;gap:8px;padding:9px 11px;border-radius:10px;background:rgba(47,158,99,0.08);border:1px solid rgba(47,158,99,0.2);font-size:12px;color:#2f9e63">
            ...
            {{ t('app.convoy_bonus', { cash: Math.round((store.rewardMultiplier.cash - 1) * 100), xp: Math.round((store.rewardMultiplier.xp - 1) * 100) }) }}
          </div>
          ...
          <div v-if="store.missionProgress" style="padding:10px 11px;border-radius:10px;background:#f6f7f8;border:1px solid #eef0f2">
            <div style="display:flex;align-items:center;justify-content:space-between;font-family:'IBM Plex Mono',monospace;font-size:11px;color:#6b7280">
              <span>{{ t('app.pool_progress_label') }}</span>
              <span>{{ t('app.total_count', { total: store.missionProgress.totalPallets }) }}</span>
            </div>
            ...
            <div style="font-family:'IBM Plex Mono',monospace;font-size:10px;color:#9aa1ab;margin-top:5px">{{ t('app.claimed_delivered', { claimed: store.missionProgress.claimedTotal, delivered: store.missionProgress.deliveredTotal }) }}</div>
          </div>
            ...
            >
              ...
              <div style="flex:1;min-width:0">
                ...
                <div style="font-family:'IBM Plex Mono',monospace;font-size:9px;color:#9aa1ab;display:inline-flex;align-items:center;gap:5px">
                  <span style="width:6px;height:6px;border-radius:50%" :style="{ background: m.online ? '#2f9e63' : '#aab0b8' }"></span>{{ m.online ? t('app.online') : t('app.offline') }}
                </div>
              </div>
              <template v-if="isLeader && !isSelf(m)">
                <button
                  v-if="m.online"
                  :title="t('app.make_leader')"
          ...
          >{{ isLeader ? t('app.disband_convoy') : t('app.leave_convoy') }}</button>
        </template>

        <!-- Not in a convoy -->
        <template v-else-if="!store.pendingInvite">
          <div style="text-align:center;padding:14px 4px;color:#9aa1ab;font-size:13px">{{ t('app.not_in_convoy') }}</div>

          <div style="display:flex;align-items:center;justify-content:space-between">
            <span style="font-size:12px;font-weight:600;color:#1b1f24">{{ t('app.invite_company_member') }}</span>
            <button
          ...
          >
            ...
            <div style="flex:1;min-width:0">
              ...
              <div style="font-family:'IBM Plex Mono',monospace;font-size:9px;color:#9aa1ab">{{ t('app.level_short', { lvl: c.lvl }) }}</div>
            </div>
            <button class="accent-btn" style="padding:6px 11px;font-size:11px;flex-shrink:0" @click="invite(c.identifier)">{{ t('app.invite') }}</button>
          </div>
          <div v-if="!candidatesLoading && candidatesFetched && candidates.length === 0" style="padding:10px 0;text-align:center;font-size:12px;color:#9aa1ab">{{ t('app.no_online_company_members') }}</div>
        </template>
